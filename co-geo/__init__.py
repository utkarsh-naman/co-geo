...
def intersect2(x1, y1, z1, x2, y2, z2, a1n, a1d, b1n, b1d, c1n, c1d, a2n, a2d, b2n, b2d, c2n, c2d):
    # print("---------intersect2-----------")
    x1 = eval(tte(x1))
    x2 = eval(tte(x2))
    y1 = eval(tte(y1))
...
def intersect(x1, y1, z1, x2, y2, z2, a1, b1, c1, a2, b2, c2):
    x1 = eval(tte(x1))
    x2 = eval(tte(x2))
    y1 = eval(tte(y1))
    y2 = eval(tte(y2))
    ...
    if a1 == a2 and b1 == b2 and c1 == c2:
        return "same line", "same line", "same line"
    else:

        if (a1 * b2 - a2 * b1) != 0:
            l = (a2 * (y1 - y2) - b2 * (x1 - x2)) / (a1 * b2 - a2 * b1)

        if (a1 * c2 - a2 * c1) != 0:
            l2 = (a2 * (z1 - z2) - c2 * (x1 - x2)) / (a1 * c2 - a2 * c1)

        if (b1 * c2 - b2 * c1) != 0:
            l3 = (b2 * (z1 - z2) - c2 * (y1 - y2)) / (b1 * c2 - b2 * c1)
        ...
        if l == l2 == l3:
            ...
            return "doesn't intersect", "doesn't intersect", "doesn't intersect"
...
class Excentre_3d:
    def __init__(self, x1, y1, z1, x2, y2, z2, x3, y3, z3):
        ...
        if self.x1 == self.x2 == self.x3 and self.y1 == self.y2 == self.y3 and self.z1 == self.z2 == self.z3:
            ...
            self.x = (-1 * self.a * self.x1 + self.b * self.x2 + self.c * self.x3) / self.p1
            self.x_fraction = f"{(-1 * self.a * self.x1 + self.b * self.x2 + self.c * self.x3)}/{self.p1}"
            self.y = (self.a * self.y1 - self.b * self.y2 + self.c * self.y3) / self.p2
            self.y_fraction = f"{(self.a * self.y1 - self.b * self.y2 + self.c * self.y3)}/{self.p2}"
            self.z = (self.a * self.z1 + self.b * self.z2 - self.c * self.z3) / self.p3
            self.z_fraction = f"{(self.a * self.y1 + self.b * self.y2 - self.c * self.y3)}/{self.p3}"
            self.ar = area_3d(self.x1, self.y1, self.z1, self.x2, self.y2, self.z2, self.x3, self.y3, self.z3)
...
class Orthocentre_3d:
    def __init__(self, x1, y1, z1, x2, y2, z2, x3, y3, z3):
        ...
        self.x, self.y, self.z, self.x_fraction, self.y_fraction, self.z_fraction = intersect2(str(self.x3),
                                                                                               str(self.y3),
                                                                                               str(self.z3),
                                                                                               str(self.x1),
                                                                                               str(self.y1),
...
while 1 > 0:
    ...
    # x, y, z = intersect2("3","2","-4", "5","-2","0", "1","2","2", "3","2","6")
    # print(x)
    # print(y)
    # print(z)
